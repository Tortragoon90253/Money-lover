/**
 * Gemini Proxy — เลขาผู้ช่วยวิเคราะห์ (Money Lover Analytics)
 * ─────────────────────────────────────────────────────────
 * หลักการ: เบราว์เซอร์ "ไม่ถือ" Gemini API key เลย
 *   เบราว์เซอร์ ──► ฟังก์ชันนี้ (ถือ key + ยาม) ──► Google Gemini
 *
 * เหมาะกับการแชร์ให้คนอื่นใช้ AI ได้โดยไม่ต้องขอ key เอง
 * (โหมด "คีย์ส่วนตัว" ในแอปยังใช้ได้ตามปกติ — proxy เป็นทางเลือกเสริม)
 *
 * ยามที่ใส่ไว้:
 ...
 *   ACCESS_CODES  = รหัสเข้าใช้ คั่นด้วย comma เช่น "myhome,friend2026"
...
const MAX_BODY = 64 * 1024;                     // ~64KB (พอสำหรับข้อมูลสรุป — แอปนี้ส่งเฉพาะยอดสรุป ไม่มีรูป)
...
exports.askAI = onRequest(
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (rawLen > MAX_BODY) {
        res.status(413).json({error: {message: 'คำขอใหญ่เกินไป'}}); return;
      }
      const body = req.body || {};
